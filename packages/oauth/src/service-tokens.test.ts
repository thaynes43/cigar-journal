...
describe("service tokens", () => {
  ...
  it("expires ttlDays out, defaulting to a year, and refuses an implausible TTL", async () => {
    ...
    for (const ttlDays of [0, -1, 731, 1.5]) {
  ...
  it("leaves the service client unusable by the browser flow — no AS change needed", async () => {
    const minted = await mint();
    // The empty registered redirect set rejects EVERY redirect_uri, so the flow
    // cannot start. This is what closes it — not the empty grant_types.
    ...
    // And no code can exist for it, so the token endpoint dead-ends.
