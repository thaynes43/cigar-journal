...
// only this drop's own. Its LINKS live in `photo_drop_tokens` (migration 0040):
// only the SHA-256 hash of a URL token is ever stored, never the raw token, so
// re-opening a drop must MINT another one — and since #316 the earlier links are
// left valid, up to PHOTO_DROP_TOKENS_MAX per drop.
//
...
// the remainder is swept. The authoritative DDL (the partial open-drop and smoke
// indexes) lives in migration 0033.
