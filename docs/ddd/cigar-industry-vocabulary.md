...
| **Sampler** | A retailer assortment spanning brands, blends or sizes. Vendor merchandising, with no authoritative definition. Its siblings — `Mix & Match`, `Bundle Deal`, a `Trio Deal`, two marcas joined by `&` — are the same fact under other words. | Matches **no single leaf**. A crawl counts one as skipped-non-cigar (#164): not a triage row, since the only resolution a curator could give it is "not a cigar". `save_smoke` and `add_cigar` refuse one and ask which cigar was smoked; `record_purchase` records it, because a sampler is bought as a unit. |
