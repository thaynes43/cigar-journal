...
| **Photo Drop** | A User's link for the smoke in progress (ADR-014): opened before the Smoke exists, multi-use for 48 hours, holding **staged** photos until a `save_smoke` names it and claims them onto the new Smoke. One open Drop per **session** — a re-open past `DROP_SESSION_GAP_HOURS` opens a new Drop and leaves the old one under its own link; within the session, or on an explicit resume by id, the Drop comes back with another token and every earlier link keeps working (issue #316), up to `PHOTO_DROP_TOKENS_MAX` per Drop. Reading a Drop (`get_photo_drop`) changes nothing. Nothing is claimed that a caller did not name. |
