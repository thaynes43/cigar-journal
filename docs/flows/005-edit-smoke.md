...
sequenceDiagram
    ...
    W->>A: tRPC updateSmoke { smokeId, version: 2, patch }
...
then calls `update_smoke` with a field-scoped patch (see tool contract).
No version precondition for MCP patches — targeted fields, last-write-wins,
audit-trailed (ADR-002 rationale).
...
- `stale_version` (web) → reload and re-apply; never silent overwrite.
